...
class Engine(object):
    ...
    def main(self):
        ...
        if self.option.use_gpu and torch.cuda.is_available():
            ...
            center_model = center_model.cuda()
    ...
    def on_end_epoch(self, option, state, epoch, model_hash, model_center, is_best, optimizer_hash, optimizer_center):
        ...
        model_dict = {
            'epoch': epoch,
            'model_hash_dict': model_hash.state_dict() if option.use_gpu and torch.cuda.is_available() else model_hash.state_dict(),
            'model_center_dict': model_center.state_dict() if option.use_gpu and torch.cuda.is_available() else model_center.state_dict(),
    ...
    def train(self, model, train_loader, criterion, hash_center_pre, optimizer, centerWeight, epoch):
        ...
        for i, (input, target) in enumerate(train_loader):
            ...
            if not self.option.fixed_weight:
                ...
                loss = criterion(hash_code.detach(), hash_centroid, hash_center_pre.detach(), centerWeight_batch, target)
